...
public:
    icmp6_server(struct session* session)
        : transport_base(session->verbose), icmp6_base(session->pcap, session->cap_dev, session->random_id), local_addr(*(struct sockaddr_in6*)&session->local_addr)
    {
    }

    icmp6_server(struct sockaddr_in6 local_addr, bool pcap = false, char *cap_dev = NULL, bool random_id = false, bool verbose = false)
        : transport_base(verbose), icmp6_base(pcap, cap_dev, random_id), local_addr(local_addr)
    {
    }
