...
int udp_icmp_server_to_remote_loop(struct session *s)
{
    int res;
...
int udp_icmp_remote_to_server_loop(struct session *s)
{
    int res;
    char buffer[MTU_SIZE];
    socklen_t addrlen = IP_SIZE;
...
#if HAVE_PCAP
int udp_icmp_remote_to_server_pcap_loop(struct session *s)
{
    int res;
