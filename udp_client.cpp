...
public:
    udp_client(struct sockaddr_in remote_addr, bool tls, struct session* session)
        : transport_base(session->verbose), udp_base(tls), remote_addr(remote_addr), tls_no_verify(session->tls_no_verify), tls_ca_path(session->tls_ca_bundle)
    {
    }

    udp_client(struct sockaddr_in remote_addr, bool tls = false, bool tls_no_verify = false, char *tls_ca_path = nullptr, bool verbose = false)
        : transport_base(verbose), udp_base(tls), remote_addr(remote_addr), tls_no_verify(tls_no_verify), tls_ca_path(tls_ca_path)
    {
    }
